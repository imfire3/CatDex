/**
 * Flat form Vision schema — matches the CaptureReveal field mapping exactly.
 * Structured Outputs: additionalProperties false, all keys required.
 */
...
export const CATDEX_FORM_JSON_SCHEMA = {
  ...
  properties: {
    ...
    coatColor: {
      type: 'string',
      description: 'Visible coat color(s) in French, e.g. Roux et blanc. Empty if unknown.',
    },
    coatPattern: {
      type: 'string',
      description: 'Visible pattern if clear (tigré, bicolore…). Empty if unknown.',
    },
    furLength: {
      type: 'string',
